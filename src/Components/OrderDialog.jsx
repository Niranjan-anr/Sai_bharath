...
const OrderDialog = ({ items, totalPrice, onClose, onConfirmOrder, setCoupon }) => {
  ...
  const handleConfirmOrder = () => {
    if (typeof onConfirmOrder === 'function') {
      const address = { name, city, street, pincode, Contact };
      onConfirmOrder(address, totalPrice);
    }
  };

  const handleApplyCoupon = () => setCoupon(couponCode);

  return (
    <div className={classes.orderDialogOverlay}>
      <div className={classes.orderDialogContent}>
        <h2 className={classes.orderDialogTitle}>Confirm Your Order</h2>
        ...
        <div className={classes.orderDialogAddress}>
          ...
          <input type="text" id="city" value={city} onChange={handleCityChange} placeholder="Enter your city" />

          <label htmlFor="street">Street</label>
          <input type="text" id="street" value={street} onChange={handleStreetChange} placeholder="Enter your street" />

          <label htmlFor="Contact">Contact Number</label>
          <input type="text" id="Contact" value={Contact} onChange={handleContactChange} placeholder="Enter your contact" />

          <label htmlFor="pincode">Pincode</label>
          <input type="text" id="pincode" value={pincode} onChange={handlePincodeChange} placeholder="Enter your pincode" />
        </div>

        <div className={classes.orderDialogCoupon}>
          <label htmlFor="coupon">Coupon Code</label>
          <input type="text" id="coupon" value={couponCode} onChange={handleCouponChange} placeholder="Enter coupon code" />
          <button className={classes.orderDialogButton} onClick={handleApplyCoupon}>Apply Coupon</button>
        </div>

        <div className={classes.orderDialogActions}>
          <button className={classes.orderDialogButton} onClick={handleConfirmOrder}>Confirm Order</button>
          <button className={classes.orderDialogButtonCancel} onClick={onClose}>Cancel</button>
        </div>
      </div>
    </div>
  );
